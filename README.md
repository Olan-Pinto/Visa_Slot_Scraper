...
Automatically checks for Abu Dhabi visa appointment slots every 15 minutes and sends email notifications when slots become available.
...
4. If slots went from 0 to positive (or increased):
...
Edit `check_visa_slots.py` line 15:
```python
...
Edit `check_visa_slots.py` around line 110 to alert on any increase:
```python
...
- Email: Free with Gmail or most email providers
...
3. Test your email credentials with a simple Python script
